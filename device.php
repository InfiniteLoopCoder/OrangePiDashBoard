...
if (($str = @file("/proc/cpuinfo")) !== false){
    ...
    if (false !== is_array($model[1])){
        $D['cpu']['count'] = 4;//sizeof($model[1]);
        $bogomips[1][0] = ' | Bogomips:'.$bogomips[1][0];
        if($D['cpu']['count'] == 1){
            $D['cpu']['model'] = $model[1][0].$bogomips[1][0];
        }
        else{
            $D['cpu']['model'] = 'ARMv8 Processor rev 4 (aarch64)'.$bogomips[1][0].' ×'.$D['cpu']['count'];//$model[1][0].$bogomips[1][0].' ×'.$D['cpu']['count'];
        }
    }
}
...
//
//    //if (false !== is_array($model[1])){
//        $D['cpu']['count'] = 4;//sizeof($model[1]);不兼容特此修改
//        //$bogomips[1][0] = ' | Bogomips:'.$bogomips[1][0];
//        //if($D['cpu']['count'] == 1){
//            //$D['cpu']['model'] = $model[1][0].$bogomips[1][0];
//        //}
